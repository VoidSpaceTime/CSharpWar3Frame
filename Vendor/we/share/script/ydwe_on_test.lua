...
function event.EVENT_TEST_MAP(event_data)
    ...
    log.debug("Testing " .. current_map_path:string())
    ...
    if global_config["MapTest"]["EnableMlScript"] ~= "0" then
        local folder = global_config["MapTest"]["MlScriptFolder"]
        if folder and folder:trim() ~= "" then
            commandline = commandline .. ' -scriptrunnerfolder "' .. folder .. '"'
        end
    end
