...
{
    private static readonly Lazy<JassVM> MainJassVM = new(() => GetJassVM());

    private static JassVM GetJassVM(int i = 1)
    {
        unsafe
        {
            var addr = *(nint*)GetJassEnvAddress();
            addr = *(nint*)(addr + 0x14);
            addr = *(nint*)(addr + 0x90);
    ...
    private static JassVM GetCurrentJassVM()
    {
        unsafe
        {
            ...
            {
                addr = *(nint*)(addr + 0x0C);
                var realJassVM = *(nint*)(addr + 0x04 * (i - 1));
    ...
    {
        private static readonly Lazy<nint> CreateJassStringPtr = new(() => SelectVersion(new Select(V27A: 0x7F1540)));

        private static readonly Lazy<nint> DestroyJassStringPtr = new(() => SelectVersion(new Select(V27A: 0x7EC9B0)));

        private static readonly Lazy<nint> CreateOpcodeIdPtr = new(() => SelectVersion(new Select(V27A: 0x7ECE50)));
        private readonly nint RealJassVM = realJassVM;

        public static bool operator true(JassVM jassVM)
        {
            return jassVM != null && jassVM.RealJassVM != 0;
        }

        public static bool operator false(JassVM jassVM)
        {
            return jassVM == null || jassVM.RealJassVM == 0;
        }

        public static bool operator !(JassVM jassVM)
        {
            return jassVM == null || jassVM.RealJassVM == 0;
        }
        ...
        public string GetJassString(uint i)
        {
            unsafe
            {
                var addr = *(nint*)(RealJassVM + 0x2874);
                if (i >= *(uint*)(addr + 0x04))
                    return "";
        ...
        public uint GetJassHashedId(string str)
        {
            unsafe
            {
                var hash = GetHashKey(str);
                var addr = *(nint*)(RealJassVM + 0x2858);
                addr = *(nint*)(addr + 0x08);

                var hashMask = *(uint*)(addr + 0x34);
                var offset = (hashMask & hash) * 12 + 8;
                ...
                while (addr.ToInt32() > 0)
                {
                    if (hash == *(uint*)addr)
                    {
                        ...
                        if (namePtr != null)
                        {
                            ...
                            fixed (byte* strPtr = utf8Bytes)
                            {
                                if (CStringCompare(strPtr, namePtr)) return *(uint*)(addr + 0x18);
                            }
                        }
                    }
        ...
        private void SetHandleReference(nint handle, bool sub)
        {
            unsafe
            {
                var handleContext = *(nint*)(RealJassVM + 0x28A4);
                if (handleContext == 0)
                    return;
        ...
        public uint GetHandleReference(nint handle)
        {
            unsafe
            {
                ...
                var arrayPtr = *(nint*)handleContext;
                if (arrayPtr == 0)
                    return 0;

                var size = *(int*)(arrayPtr + 0x198);
                if (h > size)
                    return 0;

                var objectsPtr = *(nint*)(arrayPtr + 0x19C);
                if (objectsPtr == 0)
                    return 0;

                var objectPtr = objectsPtr + h * 12;
