...
public static class AbilityCostHelper
{
    ...
    public static bool CheckCost(Entity unit, Entity ability)
    {
        ...
        {
            float mana = UnitAttrHelper.GetCurrent(unit, AttributeHelper.Mana);
        ...
        {
            float hp = UnitAttrHelper.GetCurrent(unit, AttributeHelper.Health);
        ...
        {
            float current = UnitAttrHelper.GetCurrent(unit, attrCost.attrId);
    ...
    public static void ApplyCost(Entity unit, Entity ability)
    {
        ...
        if (ability.TryGetComponent<ManaCost>(out var manaCost))
        {
            UnitAttrHelper.ModifyCurrent(unit, AttributeHelper.Mana, -manaCost.value);
        ...
        if (ability.TryGetComponent<HealthCost>(out var hpCost))
        {
            UnitAttrHelper.ModifyCurrent(unit, AttributeHelper.Health, -hpCost.value);
        ...
        if (ability.TryGetComponent<AttributeCost>(out var attrCost))
        {
            UnitAttrHelper.ModifyCurrent(unit, attrCost.attrId, -attrCost.value);
    ...
    {
        attrEntity = UnitAttrHelper.GetAttr(unit, attrTypeId) ?? default;
