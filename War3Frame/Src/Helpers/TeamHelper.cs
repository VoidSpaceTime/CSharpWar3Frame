...
public static class TeamHelper
{
    ...
    {
        if (!Teams.ContainsKey(teamName)) Teams[teamName] = new List<int>();
        Teams[teamName].AddRange(members);

        foreach (var playerA in members)
        {
            ref var pla = ref PlayerHelper.GetPlayer(playerA);
            if (colorSync) PlayerHelper.SetColor(ref pla, color);
            if (nameSync) PlayerHelper.SetName(ref pla, teamName);
    ...
    {
        if (Teams.ContainsKey(teamName)) return Teams[teamName].Contains(playerIndex);
        return false;
    }
