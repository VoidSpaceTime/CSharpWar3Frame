...
public partial class CommandManager
{
    ...
    {
        var weExe = new[] { "KKWE.exe", "WE.exe" }.Where(p => File.Exists(Path.Combine(Config.We, p))).FirstOrDefault();
        ...
        var weClient = new[] { "worldedit.exe", "worldeditydwe.exe", "worldeditkkwe.exe" };
        var war3Count = 0;
        ...
        if (war3Count > 0) Log.Warning("提示：检测到已有WE开启中，如果你是重复调用了we命令，请保留一个进行修改!");
        // 定义临时目录和文件路径
        ...
        if (Directory.Exists(projectAssetsPath))
        {
            ...
            if (process.Count() > 1) Log.Error("地形贴图冲突[调用过" + process.First() + "的贴图，确保项目只引用过一次的地形贴图]");
            ...
            else
            {
                ...
                if (Directory.Exists(terrainDir))
                {
                    ...
                    if (!Directory.Exists(terrainArt) || !Directory.Exists(cliff)) Log.Error($"地形贴图：{terrain} 地形数据错误");
                    DirectoryExtensions.CopyDir(cliff,
                        Path.Combine(tempW3xDir, "resource", "ReplaceableTextures", "Cliff"));
        ...
        // 创建标记文件
        var wePath = new[] { "we.exe", "kkwe.exe" }.Where(p => File.Exists(Path.Combine(Config.We, p)))
            .FirstOrDefault("");
        if (wePath is not "")
        {
