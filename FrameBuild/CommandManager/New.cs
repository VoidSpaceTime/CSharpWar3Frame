...
public partial class CommandManager
{
    ...
    public bool New()
    {
        if (Directory.Exists(Config.Pwd + "/Projects") is not true) Directory.CreateDirectory(Config.Pwd + "/Projects");
        if (ProjectName.StartsWith("_"))
        {
            Log.Error("项目名不合法(下划线“_”开始的名称已被禁用)");
            return false;
        }

        if (ProjectName is "")
        {
            Log.Error("不允许存在空名称项目");
            return false;
        }
        ...
        {
            Log.Error($"项目已存在,请使用run {ProjectName} 命令直接测试");
            return false;
        }

        if (Directory.Exists(damoDir))
        {
