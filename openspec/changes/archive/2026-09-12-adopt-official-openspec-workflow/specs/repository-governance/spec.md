# Spec Delta：仓库治理（repository-governance）

> 官方格式 delta。归档时由 `openspec archive` 按 `RENAMED → REMOVED → MODIFIED → ADDED` 合入 `openspec/specs/repository-governance/spec.md`。
