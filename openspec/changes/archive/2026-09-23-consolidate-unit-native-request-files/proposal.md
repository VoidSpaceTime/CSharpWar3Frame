...
新文件头部记录一条防回归警告：`UnitCreateNativeSystem` 不得挪入本文件，原因见 Why 末段。

## 非目标

- 不改命名空间、不改 `[SystemRegister]` 的 Kind/Order、不改任何方法体。
- 不合并 `UnitCreateNativeSystem`（原因见 Why）。
...
3. 两个类的 `OnUpdate` 方法体与改动前逐行等价（62 行全部命中）。
4. `Systems/Native/` 文件数 5 → 4；系统类数不变（目录内仍 7 个）。
